...
export class CookieUtil {
     ...
     */
    static get(cookieName, cookiesStringSpecified = null, isIgnoreCase = false) {
        if (cookieName == null
            || cookieName.length < 1) {
            return null;
        }
        ...
        for (let cookieIndex = 0; cookieIndex < sourceCookieStrings.length; cookieIndex++) {
            let sourceCookieString = sourceCookieStrings[cookieIndex].trim();
            let isCookieNameMatched = false;
            if (isIgnoreCase) {
                isCookieNameMatched = StringUtil.indexOfKeywordIn(sourceCookieString, cookieName, true) == 0;
            }
            else {
                isCookieNameMatched
            ...
            if (isCookieNameMatched == true) {
                ...
                let sourceCookieName = sourceCookieString.substring(0, sourceCookieNameEndCharIndex)
                    .trim();
                if (sourceCookieName.length == cookieName.length) {
                    let cookieValue = null;
                    if (indexOfEqualMark >= 0) {
                        cookieValue = sourceCookieString.substring(indexOfEqualMark + 1, sourceCookieString.length)
                            .trim();
     ...
     */
    static parseToMap(cookiesStringSpecified = null) {
        let sourceCookiesString = cookiesStringSpecified;
        if (StringUtil.isEmpty(sourceCookiesString) == true) {
            if (ObjectUtil.isValid(document)
                && ObjectUtil.isValid(document.cookie)) {
        ...
        for (let cookieIndex = 0; cookieIndex < sourceCookieStrings.length; cookieIndex++) {
            ...
            let cookieName = sourceCookieString.substring(0, sourceCookieNameEndCharIndex)
                .trim();
            let cookieValue = StringUtil.Empty;
            if (indexOfEqualMark >= 0) {
                cookieValue = sourceCookieString.substring(indexOfEqualMark + 1, sourceCookieString.length)
                    .trim();
                cookieValue = decodeURIComponent(cookieValue);
