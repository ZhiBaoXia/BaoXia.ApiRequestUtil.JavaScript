...
class StringUtil {
     ...
     */
    static isNumberChar(character, isIncludeDot = false) {
        if (character == null
            || character.length < 1) {
            return false;
        }
     ...
     */
    static isAlphabetChar(character) {
        if (character == null
            || character.length < 1) {
            return false;
        }
     ...
     */
    static isSameChar(charA, charB, isIgnoreCase = false) {
        if (charA == charB) {
            return true;
        }
        if (!isIgnoreCase) {
     ...
     */
    static getIntegerCharsCountOf(numberString) {
     ...
     */
    static getFloatNumberDigitsOf(numberString) {
        let dotIndex = numberString.indexOf(".");
        if (dotIndex < 0) {
            return 0;
        }
     ...
     */
    static getNumberStringInfo(numberString) {
        if (StringUtil.isEmpty(numberString)) {
            return new NumberStringInfo();
        }
        numberString = numberString;
        let integerString = StringUtil.Empty;
        let floatString = StringUtil.Empty;
        let dotIndex = numberString.indexOf(".");
        if (dotIndex < 0) {
        ...
        let numberStringInfo = new NumberStringInfo(dotIndex, integerString, floatString);
        { }
     ...
     */
    static complementZeroAtIntegerCharsLeftTo(numberString, integerNumberDigits) {
     ...
     * @returns 返回整数部分补足“0”的数字字符串。
     */
    static complementZeroAtFloatCharsRightTo(numberString, floatNumberDigits) {
        let newString = numberString;
        let floatNumberDigitsNeedComplement = floatNumberDigits
     ...
     */
    static stringByFixedFloat(numberObject, floatNumberDigits, numberRoundType = NumberRoundType.Round) {
        let numberValue = 0;
        if (numberObject != null) {
            if (typeof (numberObject) == "number") {
                numberValue = numberObject;
            }
            else if (typeof (numberObject) == "string") {
                numberValue = parseFloat(numberObject);
            }
        }
        let numberFixed = NumberUtil.numberByFixed(numberValue, floatNumberDigits, numberRoundType);
        ...
        if (floatNumberDigits > 0) {
            if (StringUtil.isEmpty(numberStringInfo.floatString)) {
                ...
                for (let floatNumberDigit = 0; floatNumberDigit < floatNumberDigits; floatNumberDigit++) {
                    numberStringFixed = numberStringFixed + "0";
                }
            }
            else {
                numberStringFixed = StringUtil.complementZeroAtFloatCharsRightTo(numberStringFixed, floatNumberDigits);
            }
        }
        else {
            // !!!
    ...
    static isSameCharsFromIndexIn(stringA, stringACompareBeginCharIndex, stringB, stringBCompareBeginCharIndex, compareCharsCount, isIgnoreCase = false, isNullEqualsEmpty = true) {
        if (stringA == null
            && stringB == null) {
            return true;
        }
        ...
        stringA = stringA;
        stringB = stringB;
        let stringACompareEndCharIndex = stringACompareBeginCharIndex
            + compareCharsCount;
        if (stringACompareEndCharIndex > stringA.length) {
            return false;
        }
        let stringBCompareEndCharIndex = stringBCompareBeginCharIndex
            + compareCharsCount;
        if (stringBCompareEndCharIndex > stringB.length) {
            return false;
        }
        for (let charIndex = 0; charIndex < compareCharsCount; charIndex++) {
            let charCodeA = stringA.charCodeAt(stringACompareBeginCharIndex + charIndex);
            let charCodeB = stringB.charCodeAt(stringBCompareBeginCharIndex + charIndex);
            if (!StringUtil.isSameChar(charCodeA, charCodeB, isIgnoreCase)) {
                return false;
            }
        }
        return true;
     ...
     */
    static indexOfKeywordIn(str, keyword, isIgnoreCase = false, firstCharIndexToSearch = 0, lastCharIndexToSearch = -1) {
        ...
        for (let charIndex = firstCharIndexToSearch; charIndex <= maxCharIndexToSearch; charIndex++) {
            if (StringUtil.isSameCharsFromIndexIn(str, charIndex, keyword, 0, keyword.length, isIgnoreCase)) {
                return charIndex;
            }
        }
        return -1;
     ...
     */
    static lastIndexOfKeywordIn(str, keyword, isIgnoreCase = false, firstCharIndexToSearch = 0, lastCharIndexToSearch = -1) {
        if (StringUtil.isEmpty(str)
            || StringUtil.isEmpty(keyword)) {
            return -1;
        }
        ...
        for (let charIndex = lastCharIndexToSearch + 1 - keyword.length; charIndex >= firstCharIndexToSearch; charIndex--) {
            if (StringUtil.isSameCharsFromIndexIn(str, charIndex, keyword, 0, keyword.length, isIgnoreCase)) {
                return charIndex;
            }
        }
    ...
    static isBeginWithKeywordIn(str, keyword, isIgnoreCase = false) {
        if (StringUtil.indexOfKeywordIn(str, keyword, isIgnoreCase, 0, keyword != null
            ? keyword.length - 1
            : 0) == 0) {
            return true;
        }
    ...
    static isEndWithKeywordIn(str, keyword, isIgnoreCase = false) {
        let lastIndexOfKeyword = StringUtil.lastIndexOfKeywordIn(str, keyword, isIgnoreCase, (str != null && keyword != null)
     ...
     */
    static isContainsKeywordIn(str, keyword, isIgnoreCase = false) {
        return this.indexOfKeywordIn(str, keyword, isIgnoreCase) >= 0;
    }
    /**
     * 获取指定字符串的左边指定长度的子字符串。
     * @param str 指定的字符串。
     ...
     */
    static trimLeftKeywordsIn(str, keywordsNeedTrimed, isIgnoreCase = false) {
        if (StringUtil.isEmpty(str)) {
            return StringUtil.Empty;
        }
        str = str;
        ...
        for (let keywordNeedTrimed of keywordsNeedTrimed) {
            if (StringUtil.isBeginWithKeywordIn(str, keywordNeedTrimed, isIgnoreCase)) {
     ...
     * @returns 返回移除指定字符串左边的关键字后的字符串。
     */
    static trimLeftKeywordIn(str, keywordNeedTrimed, isIgnoreCase = false) {
        if (StringUtil.isEmpty(keywordNeedTrimed)) {
            return StringUtil.emptyOr(str);
        }
        return this.trimLeftKeywordsIn(str, [keywordNeedTrimed], isIgnoreCase);
    }
    /**
     * 移除指定字符串右边的关键字。
     * @param str 指定的字符串。
     ...
     */
    static trimRightKeywordsIn(str, keywordsNeedTrimed, isIgnoreCase = false) {
        if (StringUtil.isEmpty(str)) {
            return StringUtil.Empty;
        }
        str = str;
        ...
        for (let keywordNeedTrimed of keywordsNeedTrimed) {
            if (StringUtil.isEndWithKeywordIn(str, keywordNeedTrimed, isIgnoreCase)) {
                str = str.substring(0, str.length - keywordNeedTrimed.length);
            }
        }
        return str;
    }
     ...
     * @returns 返回移除指定字符串右边的关键字后的字符串。
     */
    static trimRightKeywordIn(str, keywordNeedTrimed, isIgnoreCase = false) {
        if (StringUtil.isEmpty(keywordNeedTrimed)) {
            return StringUtil.emptyOr(str);
        }
        return this.trimRightKeywordsIn(str, [keywordNeedTrimed], isIgnoreCase);
    }
    /**
     ...
     */
    static replaceKeywordsInRangesWithStringsSpecifiedIn(str, substringRanges, newSubstringsSpecified, isUseLastNewSubstringsSpecifiedToRange = false) {
        if (StringUtil.isEmpty(str)) {
            return str;
        }
        ...
        for (let substringRangeIndex = 0; substringRangeIndex <= substringRangesCount; substringRangeIndex++) {
            let substringRange = substringRangeIndex < substringRangesCount
                ? substringRanges[substringRangeIndex]
                : new StringRange(str.length, 0);
            // 1/2，填充“子字符串区域之间”的文字：
            let originalSubstringBeginCharIndex = lastSubstringRangeEndCharIndex;
            let originalSubstringEndCharIndex = substringRange.beginCharIndex;
            ...
            if (substringRange.charsCount > 0) {
                let newSubstring = StringUtil.Empty;
    ...
    static replaceAllKeywordIn(str, keyword, newValue, isIgnoreCase = false) {
        ...
        return StringUtil.replaceKeywordsInRangesWithStringsSpecifiedIn(str, keywordRanges, [newValue], true);
    }
    /**
     * 在指定的字符串中替换指定的关键字为指定的新字符串。
     * @param str 指定的字符串。
     ...
     */
    static format(formatter, ...values) {
        ...
        ////////////////////////////////////////////////
        let placeholderRanges = new Array();
        let placeholderValues = new Array();
        ...
        for (let charIndex = 0; charIndex < charsCount; charIndex++) {
            ...
            if (character == Placeholder_Escape_Char) {
                ...
                let placeholderEscapeCharEndIndex = placeholderEscapeCharBeginIndex
                    + Placeholder_Escape_Char.length;
                for (let placeholderBodyCharIndex = placeholderEscapeCharBeginIndex + 1; placeholderBodyCharIndex < charsCount; placeholderBodyCharIndex++) {
                    ...
                    if (StringUtil.isEquals(placeholderBodyChar, Placeholder_Type_Char_Percent, true)) {
                        let placeholderParamLength = placeholderTypeCharBeginIndex
                            - placeholderEscapeCharEndIndex;
                        if (placeholderParamLength == 0) {
                            ////////////////////////////////////////////////
                            // !!!
                            placeholderRange
                                = new StringRange(placeholderEscapeCharBeginIndex, placeholderTypeCharBeginIndex
                    ...
                    ////////////////////////////////////////////////
                        ...
                        let placeholderTypeChar = StringUtil.isEquals(placeholderBodyChar, Placeholder_Type_Char_Decimal, true)
                            ? Placeholder_Type_Char_Decimal
                            : Placeholder_Type_Char_Integer;
                        if (value != null) {
                            let valueString = StringUtil.Empty;
                            let placeholderParamString = formatter.substring(placeholderEscapeCharEndIndex, placeholderTypeCharBeginIndex);
                            {
                                let integerCharsCountMin = -1;
                                let integerCharsCountMax = -1;
                                // 整数类型的占位符，浮点数的位数永远为“0”
                                let floatCharsCount = 0;
                                let numberStringInfo = StringUtil.getNumberStringInfo(placeholderParamString);
                                ...
                                // !!!
                                valueString
                                    = StringUtil.stringByFixedFloat(value, floatCharsCount);
                                // !!!
                                if (integerCharsCountMin > 0) {
                                    valueString = StringUtil.complementZeroAtIntegerCharsLeftTo(valueString, integerCharsCountMin);
                                }
                                if (integerCharsCountMax > 0) {
                                    if (valueString.length > integerCharsCountMax) {
                                        valueString = valueString.substring(valueString.length - integerCharsCountMax);
                                    }
                                }
                            }
                            ////////////////////////////////////////////////
                    ...
                    ////////////////////////////////////////////////
                        ...
                        let placeholderTypeChar = Placeholder_Type_Char_Float;
                        if (value != null) {
                            let valueString = StringUtil.Empty;
                            let placeholderParamString = formatter.substring(placeholderEscapeCharEndIndex, placeholderTypeCharBeginIndex);
                            if (StringUtil.isEmpty(placeholderParamString)) {
                                ...
                                if (integerCharsCount > 0) {
                                    valueString = StringUtil.complementZeroAtIntegerCharsLeftTo(valueString, integerCharsCount);
                                }
                            }
                            ////////////////////////////////////////////////
                            // !!!
                        ...
                        if (placeholderParamLength == 0) {
                            ...
                            if (values != null
                                ...
                                if (typeof valueObject == "string") {
                                    ...
                                    valueString
                                        = valueObject != null
                                            ? valueObject === null || valueObject === void 0 ? void 0 : valueObject.toString()
                                            : StringUtil.Empty;
                                }
                            }
                            if (valueString != null) {
        ...
        if (placeholderRanges.length > 0) {
            formatter = StringUtil.replaceKeywordsInRangesWithStringsSpecifiedIn(formatter, placeholderRanges, placeholderValues);
        }
        return formatter;
    }
    /**
     ...
     */
    static joinStringsWithDelimiter(delimiter, isDelimiterConsecutiveDisable, ...strings) {
            ...
            if (isDelimiterNotEmpty
                ...
                if (isDelimiterConsecutiveDisable) {
                    ...
                    while (finalString.endsWith(delimiter)) {
                        StringUtil.right;
                        finalString = finalString.substring(0, finalString.length - delimiter.length);
                    }
